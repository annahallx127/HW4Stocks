...
public interface View {
   ...
   * specified period. If the date(s) entered is invalid, it will prompt the user back to the menu to
   * restart. This method uses the model to retrieve stock data and compute
  ...
  /**
   * Presents the user with the options to calculate the crossovers of moving averages for a
   * stock over a specified period. Uses the getCrossovers() method and user inputs to calculate
   * result. This method can help the user identify potential buy or sell signals
   * based on crossover events.  If the date(s) entered is invalid, it will prompt the
   ...
   * After the portfolio is created, it will appear on the menu screen for when they want to
